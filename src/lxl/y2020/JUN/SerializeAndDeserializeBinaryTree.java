...
public class SerializeAndDeserializeBinaryTree {
    ...
    public String serialize(TreeNode root) {
        ...
        while (!nodes.isEmpty()) {
            TreeNode index = nodes.poll();
            if (index != null) {
                ans.append(index.val + ",");
                nodes.offer(index.left);
                nodes.offer(index.right);
            } else {
                ans.append(",");
            }
        }
    ...
    public TreeNode deserialize(String data) {
        ...
        int l = splits.length;
        Integer[] items = new Integer[l];
        for (int i = 0; i < l; i++) {
            items[i] = splits[i].equals("") ? null : Integer.valueOf(splits[i]);
        }
        TreeNode base = new TreeNode(items[0]);
        int cur = 1;
        Queue<TreeNode> nodes = new LinkedList<>();
