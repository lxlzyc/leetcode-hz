...
public class ValidateIpAddress {
    ...
    private String validIPV4(String IP) {
        ...
        if (strings.length == 4) {
            for (String index : strings) {
                if (index.startsWith("-")) {
                    return "Neither";
                } else if (index.length() > 1 && index.startsWith("0")) {
                    return "Neither";
                } else {
                    try {
                        if (Integer.valueOf(index) > 255) {
                            return "Neither";
                        }
                    } catch (Exception e) {
                        return "Neither";
    ...
    private String validIPV6(String IP) {
        ...
        if (strings.length == 8) {
            for (String index : strings) {
                if (index.isEmpty() || index.length() > 4 || index.startsWith("-")) {
                    return "Neither";

                } else {
                    try {
                        Long i = Long.parseLong(index, 16);
                    } catch (Exception e) {
                        return "Neither";
                    }
                }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(validateIpAddress.validIPAddress("172.16.00.1"));
        ...
        System.out.println(validateIpAddress.validIPAddress("2001:0db8:85a3:0000:0000:8a2e:0370:7334"));
        System.out.println(validateIpAddress.validIPAddress("2001:db8:85a3:0:0:8A2E:0370:7334"));


        System.out.println(validateIpAddress.validIPAddress("02001:0db8:85a3:0000:0000:8a2e:0370:7334"));
        System.out.println(validateIpAddress.validIPAddress("2001:0db8:85a3::8A2E:0370:7334"));
        System.out.println(validateIpAddress.validIPAddress("2001:0db8:85a3:0:0:8A2E:0370:7334:"));
